...
function initNavigation() {
  ...
  // Record the initial home state so popstate can restore it
  ...
  function showFFMaxView() {
    homeView.style.display  = 'none';
    ffmaxView.style.display = '';
    window.scrollTo(0, 0);
  }

  // Tapping FF Max tile → push state + show FFMax view
  if (ffTile) {
    ffTile.addEventListener('click', () => {
      history.pushState({ view: 'ffmax' }, '');
      showFFMaxView();
    });
  }

  // On-screen back button → go back in history (triggers popstate below)
  if (backBtn) {
    backBtn.addEventListener('click', () => history.back());
  }

  // Browser/Android hardware back button — always returns cleanly to Home
  window.addEventListener('popstate', (e) => {
    if (e.state?.view === 'ffmax') {
      showFFMaxView();
    } else {
      showHomeView();
    }
...
// Uses the EXACT same HTML as buildPaidMatchCard():
// match-title-row (cyan rules box) · match-stats-grid (2×2 grid)
// match-stat · match-stat-label · match-stat-value · neon
// match-card-bottom-row · join-btn-fee · join-btn-sep
// mm-room-box (identical lock/unlock structure as renderRoomBox)
function renderDailyTournaments() {
  ...
  if (matches.length === 0) {
    container.innerHTML = '<p class="mm-empty">No custom rooms right now. Check back soon!</p>';
    return;
  }
    ...
    return `
      <article class="match-card" data-admin-id="${adminEsc(match.id)}" data-starts-at="${startsMs}">
        ...
        <div class="match-card-bottom-row">
          ...
          <button class="match-join-btn-small" data-admin-join="${adminEsc(match.id)}">
            <span class="join-btn-fee">${formatUnits(match.entryFee)}</span><span class="join-btn-sep">|</span>Join
          </button>
        </div>
