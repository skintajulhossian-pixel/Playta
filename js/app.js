...
function buildMatchCardHtml(t, options = {}) {
  const { showPerKill = true, showRoomBox = true } = options;
  ...
  return `
      <article class="match-card" data-match-id="${t.matchId}">
        ...
        <div class="match-stats-grid ${showPerKill ? '' : 'cols-3'}">
          <div class="match-stat">
...
/* Wires up the "Join Now" buttons inside any container that was
...
function bindMatchJoinButtons(container, onJoinedRerender) {
  container.querySelectorAll('.match-join-btn:not(.full):not(.registered)').forEach((btn) => {
    btn.addEventListener('click', () => {
...
function renderTournamentList() {
  ...
  scrollEl.innerHTML = tournaments
    .map((t) => buildMatchCardHtml(t, { showPerKill: true, showRoomBox: true }))
    .join('');
...
/* Home page preview: only the top 2 featured matches (the
   soonest-starting, not-yet-full ones), simplified 3-column
   stats grid (no Per Kill), no Room ID box — that only matters
   once the user is looking at a match they've actually joined. */
function renderHomeFeatured() {
  ...
  listEl.innerHTML = featured
    .map((t) => buildMatchCardHtml(t, { showPerKill: false, showRoomBox: false }))
    .join('');
