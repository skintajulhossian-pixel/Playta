...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderAdminMatches();   // reads upcoming_matches from localStorage → home page cards
  startGlobalTicker();
});
...
/* ═══════════════════════════════════════════════════════════
   ...
   Reads the 'upcoming_matches' array that admin.html writes
   to localStorage and renders live tournament cards on the
   Home page (#homeFeaturedList).

   JOIN button → saves selectedMatchId to localStorage
              → redirects the user to join.html
   ═══════════════════════════════════════════════════════════ */

const ADMIN_SLOT_CAPACITY = { Solo: 48, Duo: 24, Squad: 12 };
const ADMIN_SLOT_UNIT     = { Solo: 'players', Duo: 'teams', Squad: 'teams' };

/** Convert a datetime-local string to "DD/MM/YYYY" for the card. */
...
/** Escape admin-entered text before injecting into HTML. */
...
/**
 * Build one match card using the app's existing CSS classes.
 * The countdown span uses js-countdown + data-starts-at (ms) so
 * the existing tickAllCountdowns() ticker updates it automatically.
 */
function buildAdminMatchCard(match) {
  const type      = match.matchType || 'Squad';
  const capacity  = ADMIN_SLOT_CAPACITY[type]  || 48;
  const slotUnit  = ADMIN_SLOT_UNIT[type]      || 'slots';
  const isFree    = Number(match.entryFee) === 0;
  const startsAtMs = new Date(match.datetime).getTime();
  const msLeft    = startsAtMs - Date.now();

  const btnInner = isFree
    ? 'Join Free'
  ...
  return `
    <article class="match-card" data-admin-id="${adminEsc(match.id)}">
      ...
      <div class="match-card-top">
        <div class="match-card-id">
          <img src="images/ff-icon.png" alt="" class="match-card-logo">
          <span class="match-card-name">Free Fire Max</span>
        </div>
      ...
      <div class="match-stats-grid">
        <div class="match-stat">
          <span class="match-stat-label">Date</span>
          <span class="match-stat-value">${adminFmtDate(match.datetime)}</span>
        </div>
        <div class="match-stat">
          <span class="match-stat-label">Starts In</span>
          <span class="match-stat-value match-stat-timer js-countdown ${msLeft <= 0 ? 'is-live' : ''}"
                data-starts-at="${startsAtMs}">${msLeft <= 0 ? 'LIVE' : formatCountdown(msLeft)}</span>
        </div>
        <div class="match-stat">
          <span class="match-stat-label">Prize Pool</span>
...
/** Save matchId and navigate to join.html. */
...
/**
 * Main function — called from DOMContentLoaded.
 * If admin has created matches: replaces demo cards with real ones.
 * If no matches yet: falls through so the demo free cards remain visible.
 */
function renderAdminMatches() {
  ...
  try {
    ...
    console.error('[Playta] Could not read upcoming_matches from localStorage:', e);
  }

  // Nothing from admin yet — let the hardcoded demo cards show through
  if (matches.length === 0) return;

  // Sort soonest-first, same order as the Tournament List
  const sorted = [...matches].sort((a, b) => new Date(a.datetime) - new Date(b.datetime));
  container.innerHTML = sorted.map(buildAdminMatchCard).join('');

  // One delegated listener covers all JOIN buttons in this container
