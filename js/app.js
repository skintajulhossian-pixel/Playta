...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderMegaTournaments();
  renderDailyTournaments();
  startGlobalTicker();

  // Agar user join.html se wapas aaya hai toh tournament list auto-open karo
  if (localStorage.getItem('returnToList') === '1') {
    localStorage.removeItem('returnToList');
...
function openTournamentListView() {
  ...
  if (!view) return;
  renderDailyTournaments(); // fills tlistScroll: admin Daily matches OR demo paid fallback
  view.classList.add('show');
}
...
function handleAdminJoin(matchId) {
  ...
  localStorage.setItem('returnToList', '1'); // back button pe tournament list reopen hoga
...
// ── RENDER DAILY TOURNAMENTS ───────────────────────────────
// Targets: #tlistScroll (inside the existing #tournamentListView overlay)
// Called by openTournamentListView() every time FF Max is tapped.
//
// Admin Daily matches exist  → clear tlistScroll, render admin cards
// No admin Daily matches     → render demo paid cards (same as before)
// NO empty state message ever — page always shows cards.
function renderDailyTournaments() {
  const container = document.getElementById('tlistScroll');
  if (!container) return;

  // Always clear first — removes hardcoded dummy data
  container.innerHTML = '';
  ...
  // ── No admin matches → show demo paid cards as fallback ──
  ...
  // ── Admin matches exist → render with exact buildPaidMatchCard structure ──
  container.innerHTML = adminMatches.map(match => {
    ...
    // Identical structure to buildPaidMatchCard()
    return `
      <article class="match-card" data-admin-id="${adminEsc(match.id)}" data-starts-at="${startsMs}">
        <div class="match-card-top">
          <div class="match-card-id">
        ...
        <div class="match-stats-grid">
          ...
          <div class="match-stat">
            <span class="match-stat-label">Starts In</span>
            <span class="match-stat-value match-stat-timer js-countdown"
                  data-starts-at="${startsMs}">${formatCountdown(msLeft)}</span>
          </div>
          <div class="match-stat">
            <span class="match-stat-label">Prize Pool</span>
        ...
        <div class="match-card-bottom-row">
          <div class="match-progress-wrap flex-grow">
            <div class="match-progress-labels">
              <span>0/${capacity} joined</span>
              <span class="match-spots-left">${capacity} spots left</span>
            </div>
            <div class="match-progress-track">
              <div class="match-progress-fill" style="width:0%"></div>
            </div>
          ...
          <button class="match-join-btn-small" data-admin-join="${adminEsc(match.id)}">
            <span class="join-btn-fee">${formatUnits(match.entryFee)}</span>
            <span class="join-btn-sep">|</span>Join Match
          </button>
        </div>
