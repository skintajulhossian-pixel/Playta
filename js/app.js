...
document.addEventListener('DOMContentLoaded', () => {
  loadUserSession();        /* ← Load session data into profile UI */
  initSplashScreen();
  initHeroCarousel();
  initGameSwitcher();
  initBottomNav();
...
/* ─────────────────────────────────────────
   AUTH — Session & Logout
   Reads playta_user_sess set by auth/auth.js
   and populates the profile header.
   ───────────────────────────────────────── */

function loadUserSession() {
  try {
    const raw  = localStorage.getItem('playta_user_sess');
    if (!raw) return;
    ...
    const displayName = sess.inGameName || sess.email?.split('@')[0] || 'Gamer';
    ...
    /* Top bar "Hello, Gamer!" text if present */
    const helloEl = document.querySelector('.hello-name, .user-name, #helloName');
    if (helloEl) helloEl.textContent = displayName;
...
function handleLogout() {
  /* Clear user session */
  localStorage.removeItem('playta_user_sess');
  ...
  container.querySelectorAll('.match-join-btn-small:not(.full):not(.registered)').forEach((btn) => {
    btn.addEventListener('click', (event) => {
      ...
      const matchForJoin = {
        ...
        prizePool:  match.totalPrizePool,
        perKill:    match.perKillPrize,
        map:        match.matchTitle || '',
        category:   match.category || 'daily'
      };

      /* ── 3. Save to upcoming_matches localStorage ── */
      let upcoming = [];
