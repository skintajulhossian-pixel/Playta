...
function buildPaidMatchCard(t) {
  ...
  return `
      <article class="match-card" data-match-id="${t.matchId}" data-starts-at="${t.startsAt}">
        ...
        <div class="match-stats-grid">
          ...
          <div class="match-stat">
            ...
            <div class="pk-wrap">
              ...
              <button class="pk-arrow-btn" onclick="togglePrizeBreakdown(this,'${t.matchId}')" aria-label="View prize breakdown" type="button">
                <svg class="pk-arrow-svg" width="10" height="7" viewBox="0 0 10 7" fill="none">
                  <path d="M1 1L5 5.5L9 1" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </button>
            </div>
          </div>
        ...
        <div class="prize-breakdown-panel" id="pb-${t.matchId}">
          <div class="pb-header">
            <span class="pb-title">🏅 Prize Distribution</span>
          </div>
          <div class="pb-rows">
            ${buildPrizeBreakdownRows(t)}
          </div>
        </div>
...
/* ════════════════════════════════════════════════
   ...
   Builds rows for the per-kill dropdown panel.
   Uses admin-set prizeDistribution if available,
   otherwise generates a default % split.
   ════════════════════════════════════════════════ */
function buildPrizeBreakdownRows(t) {
  const dist = t.prizeDistribution;

  /* Use admin-set distribution if it exists and has entries */
  if (dist && dist.length > 0) {
    return dist.map(d => `
      <div class="pb-row">
        <span class="pb-rank">${escapeHtml(String(d.rank))}</span>
        <span class="pb-amount">${formatUnits(d.amount)}</span>
      </div>`).join('') +
      `<div class="pb-row pb-row-perkill">
        <span class="pb-rank">+ Per Kill Bonus</span>
        <span class="pb-amount">${formatUnits(getDisplayPerKill(t.entryFee))} / kill</span>
      </div>`;
  }

  /* Default breakdown: estimated % split from prize pool */
  const pool = t.totalPrizePool || 0;
  const defaults = [
    { label: 'Booyah (Rank 1)', pct: 50 },
    { label: 'Rank 2',          pct: 25 },
    { label: 'Rank 3',          pct: 15 },
    { label: 'Rank 4 – 5',      pct: 10 }
  ];
  return defaults.map(d => `
    <div class="pb-row">
      <span class="pb-rank">${d.label}</span>
      <span class="pb-amount">${formatUnits(Math.round(pool * d.pct / 100))}</span>
    </div>`).join('') +
    `<div class="pb-row pb-row-perkill">
      <span class="pb-rank">+ Per Kill Bonus</span>
      <span class="pb-amount">${formatUnits(getDisplayPerKill(t.entryFee))} / kill</span>
...
/* Tiny helper used inside buildPrizeBreakdownRows */
...
/* Toggle the breakdown dropdown open / closed */
function togglePrizeBreakdown(btn, matchId) {
  const panel = document.getElementById('pb-' + matchId);
  if (!panel) return;

  const isOpen = panel.classList.contains('pb-open');

  /* Close every other open panel first */
  document.querySelectorAll('.prize-breakdown-panel.pb-open').forEach(p => {
    p.classList.remove('pb-open');
    const arrow = p.previousElementSibling?.querySelector('.pk-arrow-btn');
    if (arrow) arrow.classList.remove('pk-open');
  });
  document.querySelectorAll('.pk-arrow-btn.pk-open').forEach(b => b.classList.remove('pk-open'));

  /* Open this one (unless it was already open — then leave it closed) */
  if (!isOpen) {
    panel.classList.add('pb-open');
    btn.classList.add('pk-open');
    /* Smooth scroll so the panel is visible on small screens */
    setTimeout(() => panel.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 50);
  }
}

/* Close dropdown when clicking anywhere outside a match card */
document.addEventListener('click', (e) => {
  if (!e.target.closest('.match-card')) {
    document.querySelectorAll('.prize-breakdown-panel.pb-open').forEach(p => p.classList.remove('pb-open'));
    document.querySelectorAll('.pk-arrow-btn.pk-open').forEach(b => b.classList.remove('pk-open'));
  }
});
