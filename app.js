...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderAdminMatches();
  initDailyRoomsToggle();  // Free Fire card → show Daily Rooms / Back → hide
  startGlobalTicker();
});
...
/* ═══════════════════════════════════════════════════════════
   FREE FIRE CARD → DAILY ROOMS TOGGLE
   ─────────────────────────────────────────────────────────
   Home view:  Game Grid + Live Tournaments visible
               Daily Custom Rooms hidden
   FF card tap: Game Grid + Live Tournaments hide
               Daily Custom Rooms show
   Back button: reverses back to home view
   ═══════════════════════════════════════════════════════════ */
function initDailyRoomsToggle() {
  const ffTile       = document.getElementById('ffGameTile');
  const gameGrid     = document.getElementById('gameGridSection');
  const liveSec      = document.getElementById('liveTournamentsSection');
  const dailySec     = document.getElementById('dailyRoomsSection');
  const backBtn      = document.getElementById('dailyBackBtn');

  if (!ffTile || !dailySec) return;

  // ── Show Daily Rooms view ──
  ffTile.addEventListener('click', () => {
    if (gameGrid) gameGrid.style.display  = 'none';
    if (liveSec)  liveSec.style.display   = 'none';
    dailySec.style.display = '';
    // Scroll to top so the Daily Rooms header is visible
    document.getElementById('home-section')?.scrollTo(0, 0);
    window.scrollTo(0, 0);
  });
  ...
  if (backBtn) {
    backBtn.addEventListener('click', () => {
      ...
      if (gameGrid) gameGrid.style.display = '';
...
/* ═══════════════════════════════════════════════════════════
   ...
   ─────────────────────────────────────────────────────────
   Reads  upcoming_matches  from localStorage (written by
   admin.html) and renders two separate UIs:

     Mega Tournament  → #featured-tournaments  (premium neon card)
     Daily Custom Room → #admin-matches-list   (compact list card)

   Before any rendering, the 20-Minute Rule purges matches
   that ended more than 20 minutes ago.
   ═══════════════════════════════════════════════════════════ */
...
// ── 1. PURGE EXPIRED MATCHES — the 20-minute rule ──────────
//
// A Free Fire match lasts ~20 minutes. Once that window has
// passed after the scheduled start time, the match is over
// and must be removed so the home page stays clean.
...
function purgeExpiredMatches() {
  ...
  try {
    ...
    console.error('[Playta] purgeExpiredMatches: parse error', e);
    return [];
  }

  const now = Date.now();
  const EXPIRY_MS = 20 * 60 * 1000; // 20 minutes in milliseconds

  const active = matches.filter(m => {
    const startMs = new Date(m.datetime).getTime();
    // Keep the match only while:  now  ≤  startTime + 20 min
    return now <= startMs + EXPIRY_MS;
  });
  ...
  if (active.length !== matches.length) {
    ...
    console.log(`[Playta] Purged ${matches.length - active.length} expired match(es)`);
  }

  return active;
}

// ── 2. MEGA TOURNAMENT CARD — premium neon UI ──────────────
//
// Matches the existing buildFreeMatchCard() design exactly:
// big neon prize pool, Free/Paid entry badge, live countdown,
// join progress bar, room-lock section at bottom.
// The `js-countdown` class + `data-starts-at` ms attribute
// lets the existing tickAllCountdowns() update it automatically.
...
function buildMegaTournamentCard(match) {
  ...
  // Derive a 6-digit display ID from the match timestamp
  const matchNum  = '#' + (match.id?.split('_')[1]?.slice(-6) || '000000');
  // Total player capacity is always 48 for admin matches
  const capacity  = 48;
  ...
  return `
    <article class="match-card" data-admin-id="${adminEsc(match.id)}" data-starts-at="${startsMs}">
      ...
      <!-- Top: game logo · match number · match-type badge -->
      <div class="match-card-top">
        <div class="match-card-id">
          <img src="images/ff-icon.png" alt="" class="match-card-logo">
      ...
      <!-- Prize pool (large neon) + entry badge -->
      <div class="free-card-grid">
        <div class="free-card-prize">
          <span class="free-card-prize-label">Prize Pool</span>
      ...
      <!-- Date (left) + live countdown (right) -->
      <div class="free-card-datetime">
        <div class="free-card-dt-col">
          <span class="free-card-dt-label">Date</span>
      ...
      <!-- Slot progress bar + JOIN MATCH button -->
      <div class="match-card-bottom-row">
        <div class="match-progress-wrap flex-grow">
          <div class="match-progress-labels">
...
// ── 3. DAILY CUSTOM ROOM CARD — compact list UI ────────────
//
// A lighter card for everyday custom rooms. Shows the match
// title, a 3-stat row (prize / per-kill / entry), and a
// countdown — without the heavy neon prize treatment.
...
function buildDailyCustomCard(match) {
  ...
  return `
    <div class="daily-card" data-admin-id="${adminEsc(match.id)}">
      ...
      <!-- Title + countdown + join button in one row -->
      ...
      <!-- 3-column stats: prize / per kill / entry -->
...
//
// Called once on DOMContentLoaded.  Orchestrates the full flow:
//   a) purge expired matches
//   b) filter by category
//   c) render each group into its own container

function renderAdminMatches() {

  // Step 1 — clean up any matches that have already ended
  const matches = purgeExpiredMatches();

  if (matches.length === 0) {
    // No admin matches → demo cards remain visible, nothing more to do
    return;
  }

  // Step 2 — split by category
  // Matches created before the category field existed have no .category;
  // treat them as Mega Tournaments so they still appear on the home page.
  const megaMatches  = matches.filter(m => !m.category || m.category === 'Mega Tournament');
  const dailyMatches = matches.filter(m => m.category === 'Daily Custom Room');

  // Sort all groups: soonest start time first
  const byDate = (a, b) => new Date(a.datetime) - new Date(b.datetime);

  // ── Step 3 — Mega Tournaments → #featured-tournaments ──
  const featuredEl = document.getElementById('featured-tournaments');
  if (featuredEl && megaMatches.length > 0) {

    // Hide the hardcoded demo cards — real admin matches take over
    const demoList = document.getElementById('homeFeaturedList');
    if (demoList) demoList.style.display = 'none';

    featuredEl.innerHTML = [...megaMatches].sort(byDate).map(buildMegaTournamentCard).join('');

    // One delegated listener covers all JOIN buttons in this block
  ...
  // ── Step 4 — Daily Custom Rooms → #admin-matches-list ──
  const dailySection = document.getElementById('dailyRoomsSection');
  const dailyListEl  = document.getElementById('admin-matches-list');

  if (dailySection && dailyListEl) {
    if (dailyMatches.length > 0) {
      dailySection.style.display = '';   // reveal the section
      dailyListEl.innerHTML = [...dailyMatches].sort(byDate).map(buildDailyCustomCard).join('');
      ...
      dailySection.style.display = 'none';  // no daily matches → hide section
    }
  }
}
