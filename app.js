...
/* ───────────────────────────────
   MY MATCHES
   Registered tournaments + Room ID/Password
   reveal logic (unlocks 1 minute before start).

   NOTE: `startsAt` below is generated relative to
   "now" (a few demo offsets) so you can see the
   locked → unlocked transition without waiting for
   a real clock date. Swap these for real match
   start timestamps from the backend later — the
   render/countdown logic itself doesn't change.
─────────────────────────────── */
...
const myMatches = [
  {
    ...
    perKillPrize: 6,
    entryFee: 5,
    startsAt: Date.now() + 20 * 1000,        // demo: starts in 20s (watch it unlock)
    roomId: '9876543',
  ...
  },
  {
    ...
    perKillPrize: 20,
    entryFee: 50,
    startsAt: Date.now() + 45 * 60 * 1000,   // demo: starts in 45 min (stays locked)
    roomId: '9876544',
...
function renderMyMatches() {
  ...
  listEl.innerHTML = myMatches.map((m) => `
    <article class="t-card" data-match-id="${m.matchId}">
      ...
      <div class="mm-room-box" id="room-${m.matchId.replace('#','')}" data-starts-at="${m.startsAt}">
        <div class="mm-room-locked">
          <span class="mm-room-lock-icon">🔒</span>
          <span class="mm-room-locked-text">Room ID and Password will be revealed 1 minute before the match.</span>
        </div>
  ...
  listEl.querySelectorAll('.mm-copy-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
...
/* Checks every match's startsAt time and flips the room
   box to "unlocked" once we're inside the 1-minute window. */
function checkRoomUnlocks() {
  document.querySelectorAll('.mm-room-box').forEach((box) => {
