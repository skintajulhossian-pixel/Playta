...
document.addEventListener('DOMContentLoaded', () => {
  ...
  startGlobalTicker();
  initTestPayoutButton(); // TEMPORARY — remove this line with the button once testing is done
});
...
/* ═══════════ TEMPORARY TEST BUTTON — DELETE AFTER TESTING ═══════════
   Wires up the floating "🔧 Test Payout" button to manually fire
   calculateAndAddWinnings(5, 6) — simulating 5 kills at a ₹6
   per-kill rate, i.e. a ₹30 payout straight to winningBalance.
   To remove: delete this function, the initTestPayoutButton()
   call above, the <button id="testPayoutBtn"> in index.html, and
   the .test-payout-btn rule in style.css.
─────────────────────────────────────────────────────────────── */
function initTestPayoutButton() {
  const btn = document.getElementById('testPayoutBtn');
  if (!btn) return;

  btn.addEventListener('click', () => {
    calculateAndAddWinnings(5, 6);
  });
}
