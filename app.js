...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initUpiPaymentModal();
  syncWalletUI();
  renderHomeTournaments();   // fills #homeFeaturedList with demo fallback cards
  renderMegaTournaments();   // clears #admin-mega-container → admin Mega cards (hides demo if found)
  renderDailyTournaments();  // clears #daily-tournaments-container → Daily Custom Room cards
  initNavigation();          // history-API nav: FF tile → ffmax-view / back → home-view
  startGlobalTicker();
});
...
/* ───────────────────────────────
   MATCH RESULTS (Results tab)
   Demo-driven for now: tapping "Simulate Match End" on a
   joined match generates a leaderboard (3 dummy players +
   the current user with a random kill count), credits ONLY
   the current user's winnings to the wallet, and moves the
   match into the Results list. Dummy players show their own
   calculated winnings for display only — the wallet never
   touches their numbers.
─────────────────────────────── */
const completedMatches = []; // { match, leaderboard: [{rank, name, kills, winnings, isUser}] }

const DUMMY_OPPONENTS = ['SniperAlpha', 'GhostRecon99', 'BlazeFury', 'NightHawk_X', 'ViperStrike', 'ShadowByte'];

/* Core rule requested: Total Winnings = Kills × Per-Kill Rate.
   This function itself only ever credits the wallet — callers
   decide whose kills get passed in, and only the logged-in
   user's kills should ever reach this function. */
function calculateAndAddWinnings(userKills, perKillRate) {
  const winnings = userKills * perKillRate;

  winningBalance += winnings;
  syncWalletUI();

  showToast(`Congratulations! ${formatUnits(winnings)} added to your Winnings Balance.`, 'ok');

  return winnings;
}

function simulateMatchEnd(match) {
  const userKills = Math.floor(Math.random() * 12) + 1; // 1–12 kills, demo only
  const userWinnings = calculateAndAddWinnings(userKills, getDisplayPerKill(match.entryFee));

  // 5 dummy opponents (plus the user = 6 total rows) so the
  // leaderboard looks like a real match. Kill counts are drawn
  // from distinct ranges so the top 3 clearly stand out from
  // the rest once everyone is ranked — their winnings are
  // calculated the same way for display only; they never touch
  // the wallet.
  const shuffled = DUMMY_OPPONENTS.slice().sort(() => Math.random() - 0.5).slice(0, 5);
  const killRanges = [
    [14, 18], // big standout play — likely to land in the top 3
    [10, 13],
    [7, 9],
    [3, 6],
    [0, 3]
  ...
  const opponents = shuffled.map((name, i) => {
    const [min, max] = killRanges[i];
    const kills = Math.floor(Math.random() * (max - min + 1)) + min;
    return { name, kills, winnings: kills * getDisplayPerKill(match.entryFee), isUser: false };
  });

  const players = [
    { name: playerProfile.ffName || 'You', kills: userKills, winnings: userWinnings, isUser: true },
    ...opponents
  ];
  ...
  completedMatches.unshift({ match, leaderboard: players });

  renderMyMatches();
}

function renderLeaderboardRow(p) {
  const rankClass = p.rank === 1 ? 'rank-gold' : p.rank === 2 ? 'rank-silver' : p.rank === 3 ? 'rank-bronze' : '';
  const medal = p.rank === 1 ? '👑' : p.rank === 2 ? '🥈' : p.rank === 3 ? '🥉' : p.rank;

  return `
    <div class="result-row ${rankClass} ${p.isUser ? 'result-row-user' : ''}">
      <span class="result-rank">${medal}</span>
      <span class="result-name">${p.name}${p.isUser ? ' <span class="result-you-tag">YOU</span>' : ''}</span>
      <span class="result-kills">${p.kills}</span>
      <span class="result-winnings">${formatUnits(p.winnings)}</span>
    </div>
  `;
}

function renderResultsTab() {
  const listEl = document.getElementById('mmResultsList');
  if (!listEl) return;

  if (completedMatches.length === 0) {
    listEl.innerHTML = '<p class="mm-empty">No results yet — your finished matches will show up here.</p>';
    return;
  }

  listEl.innerHTML = completedMatches.map(({ match, leaderboard }) => `
    <div class="result-card">
      <div class="result-card-header">
        <span class="result-card-title">Free Fire Max - Match ${match.matchId}</span>
        <span class="match-type-tag">${match.type}</span>
      </div>
      <div class="result-table">
        <div class="result-row result-header-row">
          <span class="result-rank">Rank</span>
          <span class="result-name">Player</span>
          <span class="result-kills">Kills</span>
          <span class="result-winnings">Winnings</span>
        </div>
...
function renderMyMatches() {
  ...
  const completedIds = completedMatches.map(({ match }) => match.matchId);
  const registered = tournaments.filter((t) => t.isJoined && !completedIds.includes(t.matchId));
  ...
  if (registered.length === 0) {
    ...
    listEl.innerHTML = registered.map((m) => `
      <article class="t-card" data-match-id="${m.matchId}">
        ...
        <button class="mm-simulate-btn" data-match-id="${m.matchId}">Simulate Match End (Demo)</button>
      </article>
    `).join('');

    listEl.querySelectorAll('.mm-simulate-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        const match = tournaments.find((t) => t.matchId === btn.dataset.matchId);
        if (match) simulateMatchEnd(match);
      });
    });
  }
...
/* ═══════════════════════════════════════════════════════════
   NAVIGATION — history-API based, bug-free
   ─────────────────────────────────────────────────────────
   Home View  : #home-view visible, #ffmax-view hidden
   FFMax View : #ffmax-view visible, #home-view hidden

   Uses history.pushState so the browser back button works
   correctly without any display-toggle race conditions.
   "Daily Custom Rooms" text NEVER appears in Home View.
   ═══════════════════════════════════════════════════════════ */
function initNavigation() {
  const homeView  = document.getElementById('home-view');
  const ffmaxView = document.getElementById('ffmax-view');
  const ffTile    = document.getElementById('ffGameTile');
  const backBtn   = document.getElementById('backToHomeBtn');

  if (!homeView || !ffmaxView) return;

  // FF Max tile → hide home, show ffmax
  if (ffTile) {
    ffTile.addEventListener('click', () => {
      homeView.style.display  = 'none';
      ffmaxView.style.display = '';
      window.scrollTo(0, 0);
    });
  }

  // Back button → hide ffmax, show home
  if (backBtn) {
    backBtn.addEventListener('click', () => {
      ffmaxView.style.display = 'none';
...
/* ═══════════════════════════════════════════════════════════
   ADMIN MATCH INTEGRATION
   ─────────────────────────────────────────────────────────
   Reads  upcoming_matches  from localStorage (admin.html).

   renderMegaTournaments()  → Home View  (#admin-mega-container)
   renderDailyTournaments() → FFMax View (#daily-tournaments-container)

   20-Minute Rule runs BEFORE every render via purgeExpiredMatches().
   ═══════════════════════════════════════════════════════════ */

// ── Helpers ────────────────────────────────────────────────

function adminFmtDate(datetimeStr) {
...
// ── 20-MINUTE RULE: purge expired matches ──────────────────
function purgeExpiredMatches() {
  let matches = [];
  try {
    matches = JSON.parse(localStorage.getItem('upcoming_matches')) || [];
  } catch (e) {
    return [];
  }
  const now      = Date.now();
  ...
  const active   = matches.filter(m =>
    now <= new Date(m.datetime).getTime() + EXPIRY
  );
  if (active.length !== matches.length) {
    localStorage.setItem('upcoming_matches', JSON.stringify(active));
  }
  return active;
}

// ── MEGA TOURNAMENT CARD — premium neon UI ─────────────────
function buildMegaTournamentCard(match) {
  const type     = match.matchType || 'Squad';
  const startsMs = new Date(match.datetime).getTime();
  const msLeft   = startsMs - Date.now();
  const isFree   = Number(match.entryFee) === 0;
  const matchNum = '#' + (match.id?.split('_')[1]?.slice(-6) || '000000');

  const entryBadge = isFree
    ? `<span class="free-card-entry-badge">Free Entry</span>`
    : `<span class="free-card-entry-badge" style="color:var(--text-0);background:rgba(255,255,255,0.05);border-color:var(--border);">${formatUnits(match.entryFee)} | Join</span>`;

  return `
    <article class="match-card" data-admin-id="${adminEsc(match.id)}">
      <div class="match-card-top">
        <div class="match-card-id">
          <img src="images/ff-icon.png" alt="" class="match-card-logo">
          <span class="match-card-name">Free Fire Max - Match ${adminEsc(matchNum)}</span>
        </div>
        <span class="match-type-tag admin-type-${type.toLowerCase()}">${adminEsc(type)}</span>
      </div>
      <div class="free-card-grid">
        <div class="free-card-prize">
          <span class="free-card-prize-label">Prize Pool</span>
          <span class="free-card-prize-value">${formatUnits(match.prizePool)}</span>
        </div>
        ${entryBadge}
      </div>
      <div class="free-card-datetime">
        <div class="free-card-dt-col">
          <span class="free-card-dt-label">Date</span>
          <span class="free-card-dt-value">${adminFmtDate(match.datetime)}</span>
        </div>
        <div class="free-card-dt-col align-right">
          <span class="free-card-dt-label">Starts In</span>
          <span class="free-card-dt-value js-countdown ${msLeft <= 0 ? 'is-live' : ''}"
                data-starts-at="${startsMs}">${formatCountdown(msLeft)}</span>
        </div>
      </div>
      <div class="match-card-bottom-row">
        <div class="match-progress-wrap flex-grow">
          <div class="match-progress-labels">
            <span>0/48 joined</span>
            <span class="match-spots-left">48 spots left</span>
          </div>
          <div class="match-progress-track">
            <div class="match-progress-fill" style="width:0%"></div>
          </div>
        </div>
        <button class="match-join-btn-small" data-admin-join="${adminEsc(match.id)}">
          JOIN MATCH
        </button>
      </div>
      <div class="room-lock-box">
        <span class="room-lock-icon">🔒</span>
        <p class="room-lock-text">Join the match to unlock Room ID &amp; Password.</p>
      </div>
    </article>`;
}

// ── DAILY CUSTOM ROOM CARD — compact list UI ───────────────
function buildDailyCustomCard(match) {
  const type     = match.matchType || 'Squad';
  const startsMs = new Date(match.datetime).getTime();
  const msLeft   = startsMs - Date.now();
  const isFree   = Number(match.entryFee) === 0;
  ...
  return `
    <div class="daily-card" data-admin-id="${adminEsc(match.id)}">
      <div class="daily-card-top">
        <div class="daily-card-info">
          <div class="daily-card-title">${adminEsc(match.title)}</div>
          <div class="daily-card-meta">
            <span class="match-type-tag admin-type-${type.toLowerCase()}">${adminEsc(type)}</span>
            <span class="daily-cd js-countdown ${msLeft <= 0 ? 'is-live' : ''}"
                  data-starts-at="${startsMs}">
              ${msLeft <= 0 ? '🟢 LIVE' : '⏱ ' + formatCountdown(msLeft)}
            </span>
          </div>
        </div>
        <button class="daily-join-btn" data-admin-join="${adminEsc(match.id)}">
          ${isFree ? 'FREE' : formatUnits(match.entryFee)}<br><span>JOIN</span>
        </button>
      </div>
      <div class="daily-card-stats">
        <div class="daily-stat">
          <span class="daily-stat-label">Prize Pool</span>
          <span class="daily-stat-value neon">${formatUnits(match.prizePool)}</span>
        </div>
        <div class="daily-stat">
          <span class="daily-stat-label">Per Kill</span>
          <span class="daily-stat-value">${formatUnits(match.perKill)}</span>
        </div>
        <div class="daily-stat">
          <span class="daily-stat-label">Entry</span>
          <span class="daily-stat-value">${isFree ? 'FREE' : formatUnits(match.entryFee)}</span>
        </div>
      </div>
    </div>`;
}
...
// ── JOIN REDIRECT ──────────────────────────────────────────
function handleAdminJoin(matchId) {
  localStorage.setItem('selectedMatchId', matchId);
  window.location.href = 'join.html';
}

// ── Function A: RENDER MEGA TOURNAMENTS ────────────────────
// Targets: #admin-mega-container (Home View)
// Clears container first → wipes any stale/dummy data.
// If admin Mega matches exist → renders them, hides demo fallback.
// If none → shows demo fallback (#homeFeaturedList stays visible).
// ── Function A: RENDER MEGA TOURNAMENTS ────────────────────
// Uses the EXACT same HTML as buildFreeMatchCard():
// free-card-grid · free-card-prize-label · free-card-prize-value
// free-card-datetime · free-card-dt-col · align-right · js-countdown
// match-card-bottom-row · match-progress-wrap · match-join-btn-small
function renderMegaTournaments() {
  const adminContainer = document.getElementById('admin-mega-container');
  const demoContainer  = document.getElementById('homeFeaturedList');
  if (!adminContainer) return;

  adminContainer.innerHTML = '';

  const matches = purgeExpiredMatches()
    .filter(m => !m.category || m.category === 'Mega Tournament')
    .sort((a, b) => new Date(a.datetime) - new Date(b.datetime));

  if (matches.length === 0) {
    if (demoContainer) demoContainer.style.display = '';
    return;
  }
  if (demoContainer) demoContainer.style.display = 'none';

  adminContainer.innerHTML = matches.map(match => {
    const startsMs   = new Date(match.datetime).getTime();
    const msLeft     = startsMs - Date.now();
    const isFree     = Number(match.entryFee) === 0;
    const type       = match.matchType || 'Squad';
    ...
    // ── exact same entry badge as buildFreeMatchCard ──
    const entryBadge = isFree
      ? `<span class="free-card-entry-badge">Free Entry</span>`
      : `<span class="free-card-entry-badge">${formatUnits(match.entryFee)} | Join</span>`;

    // ── exact same room box as renderRoomBox() ──
    const roomBox = match.roomId
      ? `<div class="mm-room-box state-joined" data-starts-at="${startsMs}" data-joined="true">
           <div class="mm-room-locked">
             <span class="mm-room-lock-icon">🔒</span>
             <span class="mm-room-locked-text">You are registered! Room details will be revealed exactly 1 minute before the match.</span>
           </div>
           <div class="mm-room-unlocked">
             <div class="mm-room-row">
               <span class="mm-room-label">Room ID</span>
               <span class="mm-room-value">${adminEsc(match.roomId)}</span>
             </div>
             <div class="mm-room-row">
               <span class="mm-room-label">Password</span>
               <span class="mm-room-value">${adminEsc(match.roomPassword)}</span>
             </div>
             <button class="mm-copy-btn" data-room-id="${adminEsc(match.roomId)}" data-room-password="${adminEsc(match.roomPassword)}">Copy Room Details</button>
           </div>
         </div>`
      : `<div class="mm-room-box state-unjoined" data-starts-at="${startsMs}" data-joined="false">
           <div class="mm-room-locked">
             <span class="mm-room-lock-icon">🔒</span>
             <span class="mm-room-locked-text">Join the match to unlock Room ID &amp; Password.</span>
           </div>
    ...
    // ── identical structure to buildFreeMatchCard() ──
    return `
      <article class="match-card" data-admin-id="${adminEsc(match.id)}" data-starts-at="${startsMs}">
        <div class="match-card-top">
          <div class="match-card-id">
            <img src="images/ff-icon.png" alt="" class="match-card-logo">
            <span class="match-card-name">Free Fire Max - Match #${matchNum}</span>
          </div>
          <span class="match-type-tag">${adminEsc(type)}</span>
        </div>
        ...
        <div class="free-card-datetime">
          <div class="free-card-dt-col">
            <span class="free-card-dt-label">Date</span>
            <span class="free-card-dt-value">${formatMatchDate(startsMs)}</span>
          </div>
          <div class="free-card-dt-col align-right">
            <span class="free-card-dt-label">Starts In</span>
            <span class="free-card-dt-value js-countdown" data-starts-at="${startsMs}">${formatCountdown(msLeft)}</span>
          </div>
        </div>
  ...
  adminContainer.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-admin-join]');
    if (btn) handleAdminJoin(btn.dataset.adminJoin);
    const copy = e.target.closest('.mm-copy-btn');
    if (copy) copyToClipboard(`Room ID: ${copy.dataset.roomId}\nPassword: ${copy.dataset.roomPassword}`);
  });
}

// ── Function B: RENDER DAILY TOURNAMENTS ────────────────────
// Targets #daily-tournaments-container inside #ffmax-view.
// Uses the IDENTICAL HTML as buildPaidMatchCard() — same classes,
// same room-box, same structure as Image 1.
// Fallback: if no admin Daily matches exist, renders the hardcoded
// demo paid matches so the page is never blank.
function renderDailyTournaments() {
  const container = document.getElementById('daily-tournaments-container');
  if (!container) return;

  // Always clear first — wipes hardcoded demo data
  container.innerHTML = '';

  // Get active admin Daily matches (20-min rule already applied)
  const adminMatches = purgeExpiredMatches()
    .filter(m => m.category === 'Daily Custom Room')
    .sort((a, b) => new Date(a.datetime) - new Date(b.datetime));

  // ── FALLBACK: no admin matches → show hardcoded demo paid cards ──
  if (adminMatches.length === 0) {
    const demoMatches = tournaments.filter(t => t.entryFee > 0);
    container.innerHTML = demoMatches.map(t => buildPaidMatchCard(t)).join('');
    bindMatchJoinButtons(container, () => {
      renderDailyTournaments();
      renderHomeTournaments();
      renderMyMatches();
    });
    return;
  ...
  // ── ADMIN MATCHES: render using exact buildPaidMatchCard structure ──
  container.innerHTML = adminMatches.map(match => {
    ...
    // Same button state as getJoinButtonState (not joined, not full)
    const btnCls   = '';
    const btnLabel = 'Join Match';

    // Same room box as renderRoomBox() — always locked until admin
    // sets roomId; isJoined is false since user hasn't paid yet
    const roomBox = match.roomId
      ? renderRoomBox({
          matchId:      matchNum,
          startsAt:     startsMs,
      ...
      : `<div class="mm-room-box state-unjoined"
              data-match-id="${adminEsc(matchNum)}"
              data-starts-at="${startsMs}"
              data-joined="false">
           <div class="mm-room-locked">
             <span class="mm-room-lock-icon">🔒</span>
    ...
    // ── EXACT same HTML structure as buildPaidMatchCard() ──
    return `
      <article class="match-card" data-admin-id="${adminEsc(match.id)}" data-starts-at="${startsMs}">
        <div class="match-card-top">
          <div class="match-card-id">
        ...
        <div class="match-title-row">${adminEsc(match.title)}</div>

        <div class="match-stats-grid">
          <div class="match-stat">
            <span class="match-stat-label">Date</span>
            <span class="match-stat-value">${formatMatchDate(startsMs)}</span>
          </div>
          <div class="match-stat">
            <span class="match-stat-label">Starts In</span>
            <span class="match-stat-value match-stat-timer js-countdown" data-starts-at="${startsMs}">${formatCountdown(msLeft)}</span>
          </div>
          <div class="match-stat">
            <span class="match-stat-label">Prize Pool</span>
            <span class="match-stat-value neon">${formatUnits(match.prizePool)}</span>
          </div>
          <div class="match-stat">
            <span class="match-stat-label">Per Kill</span>
            <span class="match-stat-value neon">${formatUnits(match.perKill)}</span>
          </div>
        </div>
        ...
        <div class="match-card-bottom-row">
          <div class="match-progress-wrap flex-grow">
            ...
            <div class="match-progress-track">
              <div class="match-progress-fill" style="width:${fillPct}%"></div>
            </div>
          </div>
          <button class="match-join-btn-small ${btnCls}" data-admin-join="${adminEsc(match.id)}">
            <span class="join-btn-fee">${formatUnits(match.entryFee)}</span><span class="join-btn-sep">|</span>${btnLabel}
          </button>
        </div>
  ...
  // Delegated JOIN click → save matchId → open join.html
