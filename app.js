...
function initWalletAndProfileActions() {
  ...
  if (addMoneyBtn) {
    addMoneyBtn.addEventListener('click', () => {
      setWalletToggle('add');
      // Demo top-up until Razorpay is wired in — adds 100 Units to Deposit balance
      depositBalance += 100;
      syncWalletUI();
      showToast('100 Units added to Deposit balance! (Razorpay connects here)', 'ok');
    });
  }
...
const unitBundles = [
  ...
  { id: 'master', name: 'Master Pack', baseUnits: 500, price: 500, bonusPercent: 0, ribbon: 'Most Popular', ribbonPopular: true, icons: 3 },
...
function renderUnitStore() {
  ...
  grid.innerHTML = unitBundles.map((b) => {
    ...
    const ribbonHtml = b.ribbon
      ? `<span class="store-ribbon ${b.ribbonPopular ? 'popular' : ''}">${b.ribbon}</span>`
      : '';
    ...
    return `
      <div class="store-card" data-bundle-id="${b.id}">
        ...
        <button class="store-card-buy-btn" data-bundle-id="${b.id}">₹${b.price}</button>
      </div>
    `;
  }).join('');
  ...
  grid.querySelectorAll('.store-card-buy-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (bundle) openUpiPaymentModal(bundle);
    });
  });
}
...
function initUnitStore() {
  ...
  if (buyUnitsBtn) buyUnitsBtn.addEventListener('click', openUnitStore);
  ...
  return `
      <article class="match-card" data-match-id="${t.matchId}" data-starts-at="${t.startsAt}">
        ...
        <div class="free-card-datetime">
          <span class="free-card-date">${formatMatchDate(t.startsAt)}</span>
          <span class="free-card-timer js-countdown" data-starts-at="${t.startsAt}">Starts in: ${formatCountdown(msRemaining)}</span>
        </div>
      ...
      <article class="match-card" data-match-id="${t.matchId}" data-starts-at="${t.startsAt}">
        ...
        <div class="match-stats-grid">
          ...
          <div class="match-stat">
            ...
            <span class="match-stat-value">${formatUnits(t.entryFee)}</span>
          </div>
        </div>
...
function tickAllCountdowns() {
  document.querySelectorAll('.js-countdown').forEach((el) => {
    ...
    if (el.classList.contains('free-card-timer')) {
      el.textContent = isLive ? 'LIVE NOW' : `Starts in: ${formatCountdown(msRemaining)}`;
    } else {
