...
function renderMegaTournaments() {
  ...
  // Step 2: Get active Mega matches (20-min rule applied)
  const matches = purgeExpiredMatches()
    .filter(m => !m.category || m.category === 'Mega Tournament')
    .sort((a, b) => new Date(a.datetime) - new Date(b.datetime));

  if (matches.length === 0) {
    // No admin matches — keep demo cards visible
    if (demoContainer) demoContainer.style.display = '';
    return;
  }

  // Step 3: Admin matches found — hide demo, render real cards
  if (demoContainer) demoContainer.style.display = 'none';
  adminContainer.innerHTML = matches.map(buildMegaTournamentCard).join('');

  // Delegated JOIN listener
...
// Targets: #daily-tournaments-container (FFMax View)
// Clears container first → no stale data ever persists.
function renderDailyTournaments() {
  ...
  // Step 2: Get active Daily matches (20-min rule applied)
  const matches = purgeExpiredMatches()
  ...
  // Step 3: Render Daily cards
  container.innerHTML = matches.map(buildDailyCustomCard).join('');
